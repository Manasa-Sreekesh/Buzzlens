...
function render({ report, datasets }, opts = {}) {
  ...
  // Interactive (live server): the download button hits the same-origin
  // API route. Static export: there's no server, so it links directly to
  // the sibling .data.json file staticGenerator.js writes next to the html.
  const rawDataHref = opts.interactive ? '/api/raw-data.json' : opts.rawDataFilename || null;
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .download-btn { flex: 0 0 auto; background: var(--panel); border: 1px solid var(--border); color: var(--text); border-radius: 8px; padding: 7px 14px; font-size: 12.5px; text-decoration: none; white-space: nowrap; }
...
<div class="dashboard">
  <header>
    ...
    ${
      rawDataHref
        ? `<a class="download-btn" href="${esc(rawDataHref)}" download title="Every collected comment, before AI analysis">⬇ Download raw data (JSON)</a>`
        : ''
    }
  </header>
