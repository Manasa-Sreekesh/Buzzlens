...
function sentimentBlockInner(sentiment) {
  ...
  return `${sentimentBar(sentiment)}
      <div class="sent-nums">
        <span><b>${sentiment.positive || 0}</b> positive (${pct(sentiment.positive, total)}%)</span>
        <span><b>${sentiment.negative || 0}</b> negative (${pct(sentiment.negative, total)}%)</span>
        <span><b>${sentiment.neutral || 0}</b> neutral (${pct(sentiment.neutral, total)}%)</span>
      </div>`;
}
...
function render({ report, datasets }) {
  ...
  const embeddedData = JSON.stringify({ evidenceGroups, bySource: pm.overview.bySource, totalComments: pm.overview.totalComments, sentiment: pm.overview.sentiment }).replace(/</g, '\\u003c');
...
(function () {
  ...
  function sentimentBar(sentiment) {
    var total = (sentiment.positive || 0) + (sentiment.negative || 0) + (sentiment.neutral || 0) || 1;
    var p = pct(sentiment.positive, total), n = pct(sentiment.negative, total), u = 100 - p - n;
    return '<div class="sent-bar">' +
      '<div class="sent-seg pos" style="width:' + p + '%" title="Positive ' + p + '%"></div>' +
      '<div class="sent-seg neg" style="width:' + n + '%" title="Negative ' + n + '%"></div>' +
      '<div class="sent-seg neu" style="width:' + u + '%" title="Neutral ' + u + '%"></div></div>';
  }
  ...
  function sentimentBlockInner(sentiment) {
    ...
    return sentimentBar(sentiment) +
      ...
      '<span><b>' + (sentiment.positive || 0) + '</b> positive (' + pct(sentiment.positive, total) + '%)</span>' +
      '<span><b>' + (sentiment.negative || 0) + '</b> negative (' + pct(sentiment.negative, total) + '%)</span>' +
      '<span><b>' + (sentiment.neutral || 0) + '</b> neutral (' + pct(sentiment.neutral, total) + '%)</span></div>';
  }
