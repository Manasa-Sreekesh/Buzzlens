...
// Scans every item's text (never just a capped evidence sample) for any of
// the given keyword terms — case-insensitive substring match. Shared by
// insightsFile.js (persisted at save time) and pmDashboard.js (recomputed
// at render time) so a saved insights entry's "mentions"/source breakdown
// is always a real, exhaustive count over the full dataset, not an agent
// guess or an approximation limited to whichever items it cited as evidence.
function matchByTerms(items, terms) {
  const needles = (terms || [])
  ...
  const bySource = {};
  for (const it of matched) bySource[it.source] = (bySource[it.source] || 0) + 1;
  return { mentions: new Set(matched.map((i) => i.author)).size, sourceBreakdown: bySource, matchedItems: matched };
}
