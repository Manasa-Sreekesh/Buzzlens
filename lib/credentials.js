...
function printCredentialStatus() {
  // Every var any collector can read, required or optional — REQUIRED_CREDENTIALS
  // alone would hide optional-but-useful ones like YOUTUBE_API_KEY or Reddit's
  // OAuth pair, which is exactly what a user checking "what's already set" wants to see.
  const allVars = [...new Set(Object.values(SOURCE_CRED_VARS).flat())].filter((name) => CREDENTIAL_INFO[name]);
