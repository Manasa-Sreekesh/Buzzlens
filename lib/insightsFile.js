...
// source (e.g. { youtube: 5, twitter: 2 }), and a dataset-wide breakdown on
// overview — so a run that mixed sources (e.g. YouTube + Twitter/X) leaves
// the saved .insights.json file itself demarcating what came from where,
// not just the dashboard's runtime rendering. Derived from the real items,
// never authored by the agent, so it can't drift from the actual data.
...
module.exports = { insightsFilePath, readSavedInsights, validateInsights, attachSourceBreakdown };
